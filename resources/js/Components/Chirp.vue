    ...
    <div class="flex-1">
      <div class="flex justify-between items-center">
        <div>
          ...
          <small v-if="chirp.user.id !== chirp.update_at" class="text-sm text-gray-600">&middot; edited</small>
        </div>
      ...
      <form v-if="editing" @submit.prevent="form.put(route('chirps.update', chirp.id), { onSuccess: () => editing = false })">
        <textarea v-model="form.message" class="mt-4 w-full text-gray-900 border-gray-300 focus:border-indigo-300 focus:ting focus:ring-indigo-200 focus:ring-opacity-50 rounded-m shadow-sm"></textarea>
